...
export async function clearOrgan(organModel: SpinalOrganConfigModel) {
	...
	return Promise.all(promises)
		...
		.catch((err) => {
			spinalLog.error("Failed to clear organ", err);
			process.exit(1);
		});
}

function getAllOrganReferences(organModel: SpinalOrganConfigModel): Promise<SpinalNode[]> {
