...
export class SpinalDevice extends EventEmitter {
	...
	private _listenerModel: SpinalListenerModel | undefined;
	...
	public getProfileDataByInterval(interval: number | string): IObjectId[] {
		const data = this._profileData[interval as number] || [];

		const allChildren = data.reduce((acc: IObjectId[], el) => {
	...
	public async createDeviceItemList(context: SpinalContext, deviceNode: SpinalNode, spinalBacnetValueModel: SpinalBacnetValueModel): Promise<void> {
		...
		try {
			...
			console.log(`[${deviceName}] - items creation failed due to "${(error as Error).message}"`);
	...
	private async generateNetworkEndpoints(childrenGroups: any) {
	...
	private async formatAndGroupEndpoints(deviceName: string, endpointsToCreate: IObjectId[]) {
		if (!this.device) {
		...
		const childrenGroups = lodash.groupBy(lodash.flattenDeep(objectListDetails), function (item: any) {
			return item.type;
		});
	...
	public shouldSaveTimeSeries(objectId?: IObjectId): boolean {
		if (!objectId || typeof objectId.savetimeseries === "undefined") return this._listenerModel?.saveTimeSeries?.get() || false;
		...
		if (!found) return this._listenerModel?.saveTimeSeries?.get() || false;
	...
	private _getChildrenTimeSeries(objectId: IObjectId): boolean {
		if (!objectId?.savetimeseries) return this._listenerModel?.saveTimeSeries?.get() || false;
	...
	private _groupAndFormatItems(objectListDetails: { [key: string]: string | number | boolean }[]) {
		const itemsGrouped = lodash.groupBy(objectListDetails, function (item: any) {
			return item.type;
		});
